        ...
        for epoch in itertools.count(init_epoch + 1):
            ...
            for (melG, audioG), (melD, audioD) in loader:
                ...
                sc_loss, mag_loss = stft_loss(
                    fake_audio[:, :, : audioG.size(2)].squeeze(1), audioG.squeeze(1)
                )
                loss_g = sc_loss + mag_loss

                adv_loss = torch.tensor(
                    0.0
                ).cuda()  ## To avoid Runtime Error(Expected all tensors to be on the same device, but found at least two devices)
                ...
                if any(
                    ...
                    logger.error(
                        "loss_g %.01f loss_d_avg %.01f at step %d!"
