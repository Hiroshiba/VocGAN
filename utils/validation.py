...
def validate(
    hp, args, generator, discriminator, valloader, stft_loss, criterion, writer, step
):
    generator.eval()
    discriminator.eval()
    ...
    for mel, audio in loader:
        ...
        disc_fake = discriminator(
            fake_audio[:, :, : audio.size(2)]
        )  # B, 1, T torch.Size([1, 1, 212893])
        ...
        sc_loss, mag_loss = stft_loss(
            fake_audio[:, :, : audio.size(2)].squeeze(1), audio.squeeze(1)
        )
        loss_g = sc_loss + mag_loss

        for (feats_fake, score_fake), (feats_real, score_real) in zip(
            disc_fake, disc_real
        ):
        ...
        loader.set_description(
            "g %.04f d %.04f ad %.04f| step %d" % (loss_g, loss_d, adv_loss, step)
        )
